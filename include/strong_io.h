...
namespace IO {
    ...
    class DigitalOut {
      public:
        ...
        template <DigitalPin ClockPin>
        inline void
        shift_out(DigitalOut<ClockPin> &clockPin, Peripherals::Serials::SPIBitOrder bitOrder, uint8_t value) {
    ...
    template <DigitalPin Pin, typename Mode = NoPullup>
    class DigitalIn {
      public:
        inline DigitalIn() {
            if constexpr (TimerPin<Pin>) {
                // Ensure timer is disconnected from the pin.
                Pin::TimerChannel::set_mode(Peripherals::Timers::CompareOutputMode::Mode0);
            }
        ...
        template <DigitalPin ClockPin>
        inline uint8_t shift_in(DigitalOut<ClockPin> clockPin, Peripherals::Serials::SPIBitOrder bitOrder) {
    ...
    class AnalogIn {
      public:
        inline AnalogIn() {
            if constexpr (DigitalPin<Pin>) {
                DigitalIn<Pin, NoPullup> p;
            }
        }
    ...
    class PWM {
      public:
        ...
        inline PWM(typename Pin::TimerChannel::Timer::PrescaleMode prescale,
    ...
    template <AcPositive Positive, AcNegative Negative>
    class AnalogComp {
      public:
        inline AnalogComp() {
