...
namespace Peripherals {
    ...
    namespace MCU {
        ...
        class Power {
          public:
            using SleepControl   = IOReg<uint8_t, 0x53>; // SMCR
            using GeneralControl = IOReg<uint8_t, 0x55>; // MCUCR
            using PowerReduction = IOReg<uint8_t, 0x64>; // PRR

            template <IsSame<Peripheral>... T>
            inline static void disable_peripherals(T... periphs) {
                PowerReduction::set_bits(static_cast<uint8_t>(periphs)...);
            }

            template <IsSame<Peripheral>... T>
            inline static void enable_peripherals(T... periphs) {
                PowerReduction::clear_bits(static_cast<uint8_t>(periphs)...);
            }

            inline static void set_sleep_mode(SleepMode mode) {
                uint8_t val  = static_cast<uint8_t>(mode);
                uint8_t mask = bitmask(SM2, SM1, SM0);
            ...
            inline static void move_interrupt_table_to_flash_start() {
                ...
                GeneralControl::set_value(val & static_cast<uint8_t>(~(1 << IVSEL)));
            }
    ...
    namespace Interrupts {
        ...
        class ExternalInterrupts {
          public:
            ...
            using ExternalMaskRegister     = IOReg<uint8_t, 0x3D>; // EIMSK
            using ExternalFlagRegister     = IOReg<uint8_t, 0x3C>; // EIFR

            using ChangeControlRegister = IOReg<uint8_t, 0x68>; // PCICR
            using ChangeFlagRegister    = IOReg<uint8_t, 0x3B>; // PCIFR
            ...
            inline static void set_int0_sense_mode(ExternalInterruptMode mode) {
                uint8_t val  = static_cast<uint8_t>(mode);
                uint8_t mask = bitmask(ISC00, ISC01);
            ...
            inline static void set_int1_sense_mode(ExternalInterruptMode mode) {
                ...
                uint8_t mask = bitmask(ISC10, ISC11);
                ExternalControlRegisterA::replace_bits(mask, val);
            }
    ...
    namespace Ports {
        class PortB {
          public:
            using InputRegister  = IOReg<uint8_t, 0x23>; // PIN
            using ModeRegister   = IOReg<uint8_t, 0x24>; // DDR
            using OutputRegister = IOReg<uint8_t, 0x25>; // PORT
        };

        class PortC {
          public:
            using InputRegister  = IOReg<uint8_t, 0x26>; // PIN
            using ModeRegister   = IOReg<uint8_t, 0x27>; // DDR
            using OutputRegister = IOReg<uint8_t, 0x28>; // PORT
        };

        class PortD {
          public:
            using InputRegister  = IOReg<uint8_t, 0x29>; // PIN
            using ModeRegister   = IOReg<uint8_t, 0x2A>; // DDR
            using OutputRegister = IOReg<uint8_t, 0x2B>; // PORT
        };
    } // namespace Ports
    ...
    #define TIMER_2_PRESCALE_COUNT 7
        ...
        class Timer0 {
          public:
            ...
            using ControlA            = IOReg<uint8_t, 0x44>;     // TCCR0A
            using ControlB            = IOReg<uint8_t, 0x45>;     // TCCR0B
            using InterruptMask       = IOReg<uint8_t, 0x6E>;     // TIMSK0
            using InterruptFlag       = IOReg<uint8_t, 0x35>;     // TIFR0
            using CounterValue        = IOReg<CompareType, 0x46>; // TCNT0
            using GeneralTimerControl = IOReg<uint8_t, 0x43>;     // GTCCR
            ...
            using OutputCompareA                                   = IOReg<CompareType, 0x47>; // OCR0A
            ...
            using OutputCompareB                                   = IOReg<CompareType, 0x48>; // OCR0A
            ...
            inline static void set_force_compare_a() {
                ControlB::set_bit(FOC0A);
            }

            inline static void set_force_compare_b() {
                ControlB::set_bit(FOC0B);
            }
            ...
            inline static void set_prescale(PrescaleMode mode) {
                uint8_t mask = bitmask(CS00, CS01, CS02);
            ...
            inline static void set_waveform(WaveformMode mode) {
                ...
                uint8_t mask = bitmask(WGM00, WGM01);
            ...
            inline static uint16_t get_prescale_value(PrescaleMode mode) {
                switch (mode) {
                    case PrescaleMode::PS1:
                        return 1;
                    case PrescaleMode::PS8:
                        return 8;
                    case PrescaleMode::PS64:
                        return 64;
                    case PrescaleMode::PS256:
                        return 256;
                    case PrescaleMode::PS1024:
                        return 1024;
                    default:
                        return 0;
                }
        ...
        class Timer1 {
          public:
            ...
            using InterruptFlag       = IOReg<uint8_t, 0x36>;     // TIFR1
            ...
            inline static void enable_capture_noise_canceller() {
                ControlB::set_bit(ICNC1);
            }

            inline static void disable_capture_noise_canceller() {
                ControlB::clear_bit(ICNC1);
            }

            inline static void set_input_capture_mode(InputCaptureMode mode) {
                if (mode == InputCaptureMode::Falling) {
                    ControlB::clear_bit(ICES1);
                } else {
                    ControlB::set_bit(ICES1);
                }
            }

            inline static void set_force_compare_a() {
                ControlC::set_bit(FOC1A);
            }

            inline static void set_force_compare_b() {
                ControlC::set_bit(FOC1B);
            }
            ...
            inline static void enable_input_caputure_interrupt() {
                InterruptMask::set_bit(ICIE1);
            }

            inline static void disable_input_caputure_interrupt() {
                InterruptMask::clear_bit(ICIE1);
            }
            ...
            inline static void set_prescale(PrescaleMode mode) {
                uint8_t mask = bitmask(CS10, CS11, CS12);
                ControlB::replace_bits(mask, static_cast<uint8_t>(mode));
            }

            inline static void set_waveform(WaveformMode mode) {
                uint8_t bits = static_cast<uint8_t>(mode);
                uint8_t mask = bitmask(WGM11, WGM10);
                ControlA::replace_bits(mask, bits & mask);
                mask = bitmask(WGM13, WGM12);
                ControlB::replace_bits(mask, bits & mask);
            }
        ...
        class Timer2 {
          public:
            ...
            using InterruptFlag       = IOReg<uint8_t, 0x37>;     // TIFR2
            ...
            inline static void set_force_compare_a() {
                ControlB::set_bit(FOC2A);
            }

            inline static void set_force_compare_b() {
                ControlB::set_bit(FOC2B);
            }
            ...
            inline static void set_prescale(PrescaleMode mode) {
                uint8_t mask = bitmask(CS22, CS21, CS20);
                ControlB::replace_bits(mask, static_cast<uint8_t>(mode));
            }

            inline static void set_waveform(WaveformMode mode) {
                uint8_t bits = static_cast<uint8_t>(mode);
                uint8_t mask = bitmask(WGM21, WGM20);
                ControlA::replace_bits(mask, bits & mask);
                mask = bitmask(WGM22);
                ControlB::replace_bits(mask, bits & mask);
            }
            ...
            inline static uint16_t get_prescale_value(PrescaleMode mode) {
                switch (mode) {
                    ...
                    case PrescaleMode::PS8:
                        return 8;
                    case PrescaleMode::PS32:
                        return 32;
                    case PrescaleMode::PS64:
                        return 64;
                    case PrescaleMode::PS128:
                        return 128;
                    case PrescaleMode::PS256:
                        return 256;
                    case PrescaleMode::PS1024:
        ...
        class ChannelA {
          public:
            ...
            inline static constexpr void set_mode(CompareOutputMode mode) {
                ...
                uint8_t mask = bitmask(mode_bit1, mode_bit0);
            ...
            inline static constexpr void set_force_compare() {
                TimerType::set_force_compare_a();
            }
        };
    ...
    namespace Analog {
        ...
        class AnalogComparator {
          public:
            ...
            using CompControl          = IOReg<uint8_t, 0x50>; // ACSR
            ...
            inline static void set_interrupt_mode(ComparatorInterruptMode mode) {
                ...
                uint8_t mask     = bitmask(ACIS1, ACIS0);
                CompControl::replace_bits(mask, int_mode);

                *CompControl::ptr() = *CompControl::ptr() | old_interrupt;
            }
        ...
        class AnalogDigitalConverter {
          public:
            ...
            inline static void set_voltage_ref(VoltageReference ref) {
                ...
                uint8_t mask = bitmask(REFS1, REFS0);
            ...
            inline static void set_channel(MultiplexerChannel channel) {
                ...
                uint8_t mask = bitmask(MUX3, MUX2, MUX1, MUX0);
            ...
            inline static uint16_t read_data() {
                uint16_t low  = DataLow::get_value();
                uint16_t high = DataHigh::get_value();
            ...
            inline static void set_prescale(ADCPrescaler prescale) {
                ...
                uint8_t mask = bitmask(ADPS2, ADPS1, ADPS0);
            ...
            inline static void set_auto_trigger(AutoTriggerSource source) {
                ...
                uint8_t mask = bitmask(ADTS2, ADTS1, ADTS0);
                ControlB::replace_bits(mask, src);
            }

            template <typename Pin>
            inline static void disable_digital_input() requires(DigitalPin<Pin> &&AdcPin<Pin>) {
                DigitalInputDisable0::set_bit(Pin::analog_channel);
            }

            template <typename Pin>
            inline static void enable_digital_input() requires(DigitalPin<Pin> &&AdcPin<Pin>) {
    ...
    namespace Serials {
        ...
        class SPI {
          public:
            using ControlRegister = IOReg<uint8_t, 0x4C>; // SPCR0
            using StatusRegister  = IOReg<uint8_t, 0x4D>; // SPSR0
            using DataRegister    = IOReg<uint8_t, 0x4E>; // SPDR0
        ...
        class USARTConfig {
          public:
            ...
            inline static void clear_transmit_complete() {
                ...
                uint8_t mask = bitmask(FE0, DOR0, UPE0, TXC0);
                uint8_t val  = bitmask(TXC0);
            ...
            inline static void enable_double_transmit_speed() {
                ...
                uint8_t mask = bitmask(FE0, DOR0, UPE0, U2X0);
                uint8_t val  = bitmask(U2X0);
                USART::ControlStatusRegisterA::replace_bits(mask, val);
            }
            ...
            inline static void enable_multi_proc() {
                // datasheet says we should alawys set FE0, DOR0, and UPE0 to 0 when writing.
                uint8_t mask = bitmask(FE0, DOR0, UPE0, MPCM0);
                uint8_t val  = bitmask(MPCM0);
                USART::ControlStatusRegisterA::replace_bits(mask, val);
            }
            ...
            inline static void set_usart_character_size(USARTCharSize size) {
                ...
                uint8_t mask = bitmask(UCSZ00, UCSZ01);
                USART::ControlStatusRegisterC::replace_bits(mask, val & mask);
        ...
        class TWI {
          public:
            ...
            inline static void set_prescaler(TWIPrescale pres) {
                ...
                uint8_t mask = bitmask(TWPS0, TWPS1);
                StatusRegister::replace_bits(mask, val);
            }

            inline static TWIStatus get_status() {
                uint8_t mask = bitmask(TWS7, TWS6, TWS5, TWS4, TWS3);
